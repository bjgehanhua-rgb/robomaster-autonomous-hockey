...
class NavigateAndGrabStick(Node):
    ...
    def __init__(self):
        ...
        self.max_safe_lateral_error = 0.40
        ...
        # Simulator-mode gripper command.
        # True means closed/holding the stick; False means open.
    ...
    def set_gripper(self, target_state):
        ...
        The original hardware code used robomaster_msgs/GripperControl.
        The lightweight simulator does not provide that action server, so this
        publishes a Bool command instead.
        """
        msg = Bool()
        msg.data = bool(target_state == GRIPPER_CLOSE)
        self.gripper_pub.publish(msg)

        state_name = "closed" if msg.data else "open"
        self.get_logger().info(f"Simulator gripper set to {state_name}.")
        time.sleep(0.2)
        return True
